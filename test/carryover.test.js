...
test('持ち越し: 登録すると増える', () => {
  ...
  q.add('third_person_s', 2);
  assertDeepEqual(q.list(), [{ pattern: 'third_person_s', level: 2, misses: 1 }]);
});

test('持ち越し: 同じ pattern を登録しても件数は増えない', () => {
  const q = createCarryOverQueue();
  q.add('plural_spelling', 2);
  q.add('plural_spelling', 2);
...
test('持ち越し: 正解したらキューから消える', () => {
  const q = createCarryOverQueue();
  q.add('be_agreement', 1);
  q.add('article_an', 1);
  q.resolve('be_agreement');
  assertDeepEqual(q.list().map((it) => it.pattern), ['article_an']);
});

test('持ち越し: 3回持ち越すと1レベル下がって残る', () => {
  const q = createCarryOverQueue();
  for (let i = 0; i < 3; i++) q.add('plural_spelling', 2);
...
test('持ち越し: Lv1 でさらに3回持ち越すと黙って消える', () => {
  const q = createCarryOverQueue();
  for (let i = 0; i < 3; i++) q.add('be_agreement', 1);
  assertEqual(q.size(), 0, 'Lv1 の持ち越しが永久に残っている');
});

test('持ち越し: 毎日つまずいても件数が増え続けない', () => {
  const q = createCarryOverQueue();
  for (let day = 0; day < 30; day++) q.add('plural_spelling', 2);
...
test('持ち越し: take が返すのは写しで、外から壊せない', () => {
  ...
  const taken = q.take();
  taken[0].level = 99;
  assertEqual(q.list()[0].level, 1, 'キューの中身が外から書き換えられた');
});
...
test('持ち越し: 壊れた中身を渡しても落ちず、使える形に直る', () => {
  const broken = [
    ...
    { level: 2 },
    { pattern: 'be_agreement', level: 1, misses: 1 }
  ];
...
test('持ち越し: 引数なし・配列以外を渡しても落ちない', () => {
  for (const bad of [undefined, null, 0, 'x', { a: 1 }]) {
    assertEqual(createCarryOverQueue(bad).size(), 0);
  }
});
