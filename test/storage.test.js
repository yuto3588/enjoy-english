...
const validState = () => ({
  ...
  carryOver: [{ pattern: 'sub_negative', level: 1, misses: 1 }],
...
test('バージョン違い: マイグレーションせず level だけ引き継ぐ', () => {
  ...
  assertEqual(loaded.level, 1, 'level が引き継がれていない');
  assertDeepEqual(loaded.history, []);
...
test('学年: 別の学年の記録は互いに影響しない', () => {
  ...
  save({ ...validState(), level: 2 }, 'e5', s);

  assertEqual(load('j3', s).level, 5);
  assertEqual(load('e5', s).level, 2);
